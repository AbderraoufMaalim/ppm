 ...
 * @author Raouf
 */
public class CaisseChaiseController implements Initializable {
...
DecimalFormat formatter = new DecimalFormat("00");
    ...
    public static CaisseChaiseController instance;
	public CaisseChaiseController(){
		instance =this;
	}
	public static CaisseChaiseController getInstance(){
		return instance;
	}
...
int chichaCount = Integer.parseInt(SessionsController.getInstance().chichaTextChaise.getText());
int moneyCount = Integer.parseInt(SessionsController.getInstance().moneyText.getText());

@FXML private Spinner<Integer> playTimeSpinner;
...
@FXML private JFXButton removeAll;
    ...
    public void changed(final ObservableValue<? extends String> observable, final String oldValue, final String newValue) {
        ...
        Pane nn = (Pane) SessionsController.getInstance().gridChaise.getChildren().get(numPoste-1);
   ...
   public void addProduct(String nom, int price, int current, int tertib, int quant, Image imgP, String type){
        ...
        if(check == false){
          SessionsController.getInstance().dataChaise.add(new produit(nom, price, current, tertib, quant, imgP, type));  
        }
        
        
        //________chicha count____________________________________
        if (type.equals("Chicha")){
        chichaCount = Integer.parseInt(SessionsController.getInstance().chichaTextChaise.getText());
        chichaCount ++;
        SessionsController.getInstance().chichaTextChaise.setText(Integer.toString(chichaCount));
        }
        ...
        tableProduct.setItems(SessionsController.getInstance().dataChaise);
   ...
   public void removeProduct(String nom, int price, int current, int tertib, String type){
      ...
      SessionsController.getInstance().moneyText.setText(Integer.toString(moneyCount));
        ...
        if(check == true){
                   SessionsController.getInstance().dataChaise.removeIf(p -> p.getNomProduit().equals(nom) && p.getCurrent() == numPoste);
         }
   ...
   @FXML
    void removeAll(ActionEvent event) {
        Alert alert1 = new Alert(Alert.AlertType.CONFIRMATION, "êtes-vous sûr de vouloir Vider la Chaise "+numPoste+" ?", ButtonType.YES, ButtonType.NO, ButtonType.CANCEL);
       ...
       SessionsController.getInstance().chichaTextChaise.setText(Integer.toString(chichaCount));
               ...
               SessionsController.getInstance().dataChaise.removeIf(p -> p.getCurrent() == numPoste);
               moneyCount = Integer.parseInt(SessionsController.getInstance().moneyText.getText());
    ...
    public void fillFields (int num, String comment) throws ClassNotFoundException{
        numPoste = num;   
     pcTxt.setText("Chaise: "+formatter.format(numPoste));
     tableProduct.setItems(SessionsController.getInstance().dataChaise);
     tableProduct.setItems(tableProduct.getItems().filtered(log -> log.getCurrent() == numPoste));
...
public String getCurrentTime(){
    ...
    public void valide(ActionEvent event) throws SQLException, ClassNotFoundException {
        ...
        String sql = "INSERT INTO session (dateSession, heureDebut, heureFin, clientId, sessionType,"
                ...
                + "('"+getCurrentDate()+"', '', '"+heureF+"', '1', 'Chaise', '"+payment+"', '"+note.getText()+"', "
                + "'Guest', 1,'','"+numPoste+"');";
...
@FXML
void pancake(ActionEvent event) throws FileNotFoundException {
...
@FXML
void teaLipton(ActionEvent event) throws FileNotFoundException {
  Image image = new Image("product icons/thé lipton.png"); 
  ...
  addProduct("Thé Lipton", pTeaL, numPoste, tertib, 1, image, "Boisson");
}
@FXML
